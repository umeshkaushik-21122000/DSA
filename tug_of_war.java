...
public class tug_of_war{
    ...
    public static void solve(int[] arr, int vidx, ArrayList<Integer> set1, ArrayList<Integer> set2, int soset1,int soset2) {
        if(vidx==arr.length){
                if(Math.abs(soset1-soset2)<mindiff&&(Math.abs(set1.size()-set2.size())==1||Math.abs(set1.size()-set2.size())==0)){
        ...
        int number=arr[vidx];
        set1.add(number);
        solve(arr,vidx+1,set1,set2,soset1+number,soset2);
        set1.remove(set1.size()-1);
        set2.add(number);
        solve(arr,vidx+1,set1,set2,soset1,soset2+number);
